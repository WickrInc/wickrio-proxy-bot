class ProxyService {
  ...
  static findUserByProxy(proxyid) {
    let findUserByProxy = this.readCredentialFile.credentials.find(usercredential => usercredential.proxyid === proxyid)
    return findUserByProxy
  }

  static findUserByID(userid) {
    let findUserByID = this.readCredentialFile.credentials.find(usercredential => usercredential.userid === userid)
    return findUserByID
  }
  ...
  static addProxyID(userid, proxyid) {
    ...
    user ?
      ...
      creds.credentials.find(usercredential => usercredential.userid === userid).proxyid.append(proxyid) :
  ...
  static list(userid) {
    let creds = this.readCredentialFile()
    let user = creds.credentials.find(usercredential => usercredential.userid === userid)
