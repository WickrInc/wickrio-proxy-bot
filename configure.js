  ...
  return new Promise((resolve, reject) => {
    var recursivePrompt = function () {
      ...
      if (
        ...
        process.env.DATABASE_ENCRYPTION_KEY !== undefined
      ) {
        ...
        token === "WHITELISTED_USERS" &&
        process.env.WHITELISTED_USERS !== undefined
      ) {
        ...
        token === "ROOM_ALIAS" &&
        process.env.ROOM_ALIAS !== undefined
      ) {
        var input = token + "=" + process.env.ROOM_ALIAS;
        config.push(input);
        i++;
        return recursivePrompt();
      }
    ...
    .then(function (answer) {
      ...
      for (var key in newObjectResult) {
        ...
        if (
          ...
          var obj = {
            value: process.env.ROOM_ALIAS,
            encrypted: false,
          };
          newObjectResult.ROOM_ALIAS = obj;
          continue;
        }
        var obj = {
      ...
      try {
        ...
        if (process.env.WICKRIO_BOT_NAME !== undefined) {
          var newName =
            "WickrIO-User-Engagement-Bot_" + process.env.WICKRIO_BOT_NAME;
        } else if (newObjectResult.WICKRIO_BOT_NAME !== undefined) {
          var newName =
            "WickrIO-User-Engagement-Bot_" +
            newObjectResult.WICKRIO_BOT_NAME.value;
        } else {
          var newName = "WickrIO-User-Engagement-Bot";
        }
...
function getCurrentValues() {
  ...
  if (pjson.apps[0].env.tokens.WICKRIO_BOT_NAME !== undefined) {
    newObjectResult["WICKRIO_BOT_NAME"] =
      pjson.apps[0].env.tokens.WICKRIO_BOT_NAME.value;
  }
  if (pjson.apps[0].env.tokens.DATABASE_ENCRYPTION_KEY !== undefined) {
    newObjectResult["DATABASE_ENCRYPTION_KEY"] =
      pjson.apps[0].env.tokens.DATABASE_ENCRYPTION_KEY.value;
  }
  if (pjson.apps[0].env.tokens.WHITELISTED_USERS !== undefined) {
    newObjectResult["WHITELISTED_USERS"] =
      pjson.apps[0].env.tokens.WHITELISTED_USERS.value;
  }
  if (pjson.apps[0].env.tokens.ROOM_ALIAS !== undefined) {
    newObjectResult["ROOM_ALIAS"] = pjson.apps[0].env.tokens.ROOM_ALIAS.value;
  }
...
function processConfigured() {
  ...
  // TODO do we need the encyption key here as well??
  if (pjson.apps[0].env.tokens.WICKRIO_BOT_NAME === undefined) {
    return false;
  }
  if (pjson.apps[0].env.tokens.WHITELISTED_USERS === undefined) {
